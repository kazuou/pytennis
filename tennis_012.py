...
field_data = (-548.5-500,2377+1000,543.5+500,-1000)
...
def location_in_view(x1, y1, z1, size_x, size_z, adj_x, adj_z):
    """ 3D座標から2D上の座標算出 """
    x1=x1/2
    y1=y1/100+10
...
def location_in_view2(x1, y1, z1, size_x, size_z, adj_x, adj_z):
    """ 3D座標から2D上の座標算出 """
    x2 = int((x1 + adj_x * y1) / (y1/10 + 1)) + 200 - int(size_x * 0.5 / (y1/10 + 1))
    z2 = int((z1 + adj_z * y1) / (y1/10 + 1)) + 150 - int(size_z * 0.5 / (y1/10 + 1))
    return (x2, z2)
...
def draw_background(adjust_x, adjust_z, counter):
    ...
    a, b = location_in_view2(0, 200,0, 0, 0, adjust_x, adjust_z) #遠景
    SURFACE.blit(image_background,(a - 300, b - 320))

    #フィールドのサイズ
    x1,y1,x2,y2 = field_data
    a, b = location_in_view(x1, y1 ,150, 0, 0, adjust_x, adjust_z) #地面
...
#   court_data = (-548.5,2377,543.5,0)
    ...
    pygame.draw.polygon(SURFACE,(0, 128, 0), #テニスコート
                        (location_in_view(x1, y1, 150, 0, 0, adjust_x, adjust_z),
                         location_in_view(x2, y1, 150, 0, 0, adjust_x, adjust_z),
                         location_in_view(x2, y2, 150, 0, 0, adjust_x, adjust_z),
                         location_in_view(x1, y2, 150, 0, 0, adjust_x, adjust_z)))
...
def draw_character(image, x, z, y, width, height, adjust_x, adjust_z):
    ...
    if width < 300: #幅の狭いキャラクターのみ影表示
        ...
        a, b = location_in_view2(x, y, 147, width, width, adjust_x, adjust_z)
...
#    else:
    ...
    a, b = location_in_view2(x, y, z, width, height, adjust_x, adjust_z)
...
def main():
    ...
    while True:
        ...
        while gameover_flag == 0:
            ...
            if counter_point > 0:
                counter_point += 1
                a, b = location_in_view2(point_x, 0,100, 0, 0, adjust_x, adjust_z)
                image = sysfont.render(
