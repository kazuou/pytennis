...
MAX_BALL_SPEED = 5
CELL_SIZE = 30
WINDOW_WIDTH = 18 * CELL_SIZE  # 動ける領域に合わせて変更
WINDOW_HEIGHT = 19 * CELL_SIZE  # 動ける領域に合わせて変更
LINE_WIDTH = 1
...
class Game:
    def __init__(self, width=18, height=19, throw_width=9, throw_height=13):
        self.width = width
        self.height = height
        self.throw_width = throw_width
        ...
        for i in range(throw_width):
            self.board[height // 2, (width - throw_width) // 2 + i] = NET  # ネットの位置
    ...
    def is_valid_throw(self, start_pos, target_pos, speed, angle):
        ...
        x_center = (self.width - self.throw_width) // 2
        y_center = (self.height - self.throw_height) // 2

        # 投げる範囲内にターゲットがいるか確認
        if not (y_center <= x1 < y_center + self.throw_height and x_center <= y1 < x_center + self.throw_width):
            return False
