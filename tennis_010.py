        ...
        if self.image_type == 1 or self.image_type == 2 or self.image_type == 10 or self.image_type == 20: #樹木、ストッパー、おむすび、家
            ...
            if self.status >= 20 and self.status <= 22: #ジャンプ動作1
                ...
                if self.y > 110:
                    ...
                    self.vx = 0
                    self.vy = 0
                    self.vz = 1 if self.vz > 0 else -1
                    self.image = self.image_frog
            if self.z <= 2 and self.vz < 0 :
                self.vz = -self.vz
                self.z = 2
            if self.z >= 198 and self.vz > 0:
                self.vz = -self.vz
                self.z = 198
            ...
            if self.z == 40:
                sound_bird_fly.play()
            if self.z <= 42 and self.vz < 0 :
                self.vz = -self.vz
                self.z = 42
            if self.z >= 84 and self.vz > 0:
                self.vz = -self.vz
                self.z = 84
...
def draw_background(adjust_x, adjust_y, counter):
    ...
    a, b = location_in_view(0, 0, 200, 0, 0, adjust_x, adjust_y) #遠景
    ...
    pygame.draw.polygon(SURFACE,(0, 128, 0), #テニスコート
                        (location_in_view(x1, 150, y1, 0, 0, adjust_x, adjust_y),
                        location_in_view(x2, 150, y1, 0, 0, adjust_x, adjust_y),
                         location_in_view(x2, 150, y2, 0, 0, adjust_x, adjust_y),
                         location_in_view(x1, 150, y2, 0, 0, adjust_x, adjust_y)))
        ...
        while gameover_flag == 0:
            ...
            if character[0].status < 200 and counter <= 5970:
                ...
                if pressed[K_RIGHT] and character[0].x < 280:
                    character[0].x += 20
                if pressed[K_UP] and character[0].z < 35:
                    character[0].z += 1
                if pressed[K_DOWN] and character[0].z > 0:
                    character[0].z -= 1
